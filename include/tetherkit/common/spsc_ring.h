 ...
 public:
  ...
  [[nodiscard]] bool TryAcquireWrite(std::size_t& index) noexcept {
    ...
    if (write_pos - cached_read_pos_ >= capacity_) [[unlikely]] {
      ...
      if (write_pos - cached_read_pos_ >= capacity_) {
        return false;
      }
    }
    index = write_pos & mask_;
    return true;
  }
  ...
  void PublishWrite() noexcept {
    write_pos_.store(write_pos_.load(std::memory_order_relaxed) + 1, std::memory_order_release);
  }
  ...
  [[nodiscard]] bool TryAcquireRead(std::size_t& index) noexcept {
    ...
    if (read_pos >= cached_write_pos_) [[unlikely]] {
      cached_write_pos_ = write_pos_.load(std::memory_order_acquire);
      if (read_pos >= cached_write_pos_) {
        return false;
      }
    }
    index = read_pos & mask_;
    return true;
  }

  /// 释放刚读完的槽位，使其可被生产者复用。
  void PublishRead() noexcept {
    read_pos_.store(read_pos_.load(std::memory_order_relaxed) + 1, std::memory_order_release);
  }
