...
| **TX 在飞 bulk OUT 传输数默认只有 4** | 高负载下 BPF 读上来的帧拿不到空闲传输，被背压丢弃（实测 6 秒内丢 1889 帧） | 能。`--tx-transfers 16` 可把丢帧降到约 1/4；根因待查 |
| **RX 侧排队深，双向并发时会拖垮 TX** | RX 满载把 RTT 从 0.4 ms 抬到 16.6 ms，TX 流的 ACK 被堵住，cwnd 塌到 11 KB → TX 仅剩约 5 Mbps | 部分能。缩小 RX 排队深度可缓解，但会牺牲 RX 峰值吞吐 |
...
3. **TX 方向才是弱侧**：只能到 ~235 Mbps，且丢帧发生在**本项目自己的 TX 路径**
   （拿不到空闲 bulk OUT 传输 → 背压丢弃）。`--tx-transfers` 默认 4 偏小，
   提到 16 后丢帧降到约 1/4。详见 BENCHMARKS.md 的「真机端到端实测」。

仍然没查清的：TX 丢帧的**根因**（只定位到现象，没定位到是 USB 提交速率受限
还是 TX 抽取线程的批量策略问题）。清单见 [BENCHMARKS.md](BENCHMARKS.md) 末节。
