...
| feth 数据路径在 **> 5~8 Gbps** 时有内核 mbuf 溢出 panic 的社区报告 | 对本项目风险很低 | RNDIS over USB 2.0 实测 200~300 Mbps，USB 3 也难超 1~2 Gbps，远低于该阈值 |

---

## 尚未实测的部分

功能层面的验证情况见 [../AGENTS.md](../AGENTS.md) 第 6 节。但**性能**方面至今
没有做过任何压测，下列数字仍然都是推算或来自调研，**未经实测**：

- BPF `write()` 的实际成本（本文用的 ~700 ns 来自调研的 `write(/dev/null, 1514)`
  实测，**不是**真实 BPF 描述符上的测量）；
- `BIOCSBATCHWRITE` 相对逐帧写的实际提速倍数；
- 端到端吞吐能否接近 USB 2.0 高速的 426 Mbps 理论上限；
- feth 数据路径的往返延迟。

待补的基准清单见 [BENCHMARKS.md](BENCHMARKS.md) 末节。
