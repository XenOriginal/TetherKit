...
> ⚠️ **状态**：全部模块已实现，197 个测试用例（7268 条断言）在
...
> 端到端已用 Android 手机压测：**RX 约 326 Mbps / TX 约 240~300 Mbps，两个方向
> 都是零重传**（USB 2.0 高速，理论上限 426 Mbps）。压测中发现的 TX 高负载丢帧
> 已定位并修复（传输池占满时误丢而非等待），双向并发 TX 从 4.8 Mbps 回到
> 87 Mbps。详见 [docs/BENCHMARKS.md](docs/BENCHMARKS.md)。
